...
```{r}
posterior <- read_csv("data_538/election-forecasts-2020/presidential_ev_probabilities_2020.csv") 
ggplot(posterior, aes(x = total_ev, y = evprob_chal)) +
  geom_col() +
  labs(title = "Posterior of a Biden Win", x = "Electoral Votes", y = "Probability") + 
...
posterior_1 <- posterior %>%
  filter(total_ev > 300)
sum(posterior_1$evprob_chal)
# 0.65%


posterior_2 <- posterior %>%
  filter(total_ev == 531)
sum(posterior_2$evprob_chal)
# 0%

posterior_3 <- posterior %>%
  filter(total_ev >= 269 & total_ev <= 289)
sum(posterior_3$evprob_chal)
# 0.095%
